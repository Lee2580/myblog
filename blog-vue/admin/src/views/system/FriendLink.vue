<template>
  <el-card class="main-card">
    ...
    >
      ...
      <el-table-column prop="linkStatus" label="展示状态" align="center" width="100">
        <template slot-scope="scope">
          <el-switch
              ...
              :active-value="1"
              :inactive-value="0"
              @change="changeLinkStatus(scope.row)"
          />
        </template>
      </el-table-column>
      <el-table-column
    ...
    <el-dialog :visible.sync="addOrEdit" width="30%">
      ...
      <el-form label-width="80px" size="medium" :model="linkForm">
        ...
        <el-form-item label="展示状态">
          <el-radio-group v-model="linkForm.linkStatus">
...
export default {
  ...
  methods: {
    ...
    deleteLink(id) {
      ...
      this.axios.delete("/api/admin/deleteFriendLink", param).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
    ...
    changeLinkStatus(link) {
      console.log(link)
      this.axios
          .put("/api/admin/changeLinkStatus", {
            id : link.id,
