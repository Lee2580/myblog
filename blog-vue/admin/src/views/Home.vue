  ...
  <div>
    ...
    <el-row :gutter="30">
      ...
      <el-col :span="6">
        <el-card>
          <div class="card-icon-container">
            <i class="iconfont el-icon-myblog-copy" style="color:#F4516C" />
          </div>
          <div class="card-desc">
            <div class="card-title">文章量</div>
            <div class="card-count">{{ blogCount }}</div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
    ...
    <el-row :gutter="20" style="margin-top:1.25rem">
      ...
      <el-col :span="16">
        <el-card>
          ...
          <div style="height:350px">
            <v-chart :options="blogRank" v-loading="loading" />
          </div>
        </el-card>
      </el-col>
      <!-- 分类数据统计 -->
      ...
      this.axios.get("/api/admin").then(({ data }) => {
        ...
        if (data.data.blogRankDTOList != null) {
          data.data.blogRankDTOList.forEach(item => {
            this.blogRank.series[0].data.push(item.viewsCount);
            this.blogRank.xAxis.data.push(item.title);
          });
        }
