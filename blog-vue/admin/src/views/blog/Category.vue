<template>
  <el-card class="main-card">
    ...
    >
      ...
      <!-- 文章量 -->
      <el-table-column prop="blogCount" label="文章量" align="center" />
      <!-- 分类创建时间 -->
      ...
      <el-table-column label="操作" width="160" align="center">
        <template slot-scope="scope">
          ...
          <el-popconfirm
            title="确定删除吗？"
            style="margin-left:1rem"
            @confirm="deleteCategory(scope.row.categoryId)"
          >
...
export default {
  ...
  methods: {
    ...
    deleteCategory(categoryId) {
      ...
      this.axios.delete("/api/admin/deleteCategories", param).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
    ...
    addOrEditCategory() {
      ...
      this.axios
        .post("/api/admin/saveOrUpdateCategory", this.categoryForm)
        .then(({ data }) => {
          if (data.status) {
            this.$notify.success({
              title: "成功",
