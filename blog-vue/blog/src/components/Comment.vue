...
export default {
  ...
  methods: {
    replyComment(index, item) {
      ...
      this.$refs.reply[index].parentCommentId = this.commentList[index].commentId;
    ...
    checkReplies(index, item) {
      this.axios
        .get("/api/comments/" + item.commentId + "/replies", {
    ...
    insertComment() {
      ...
      console.log(comment);
      this.axios.post("/api/saveComment", comment).then(({ data }) => {
        if (data.status) {
          //查询最新评论
    ...
    like(comment) {
      ...
      // 发送请求
      this.axios
        .post("/api/comments/" + comment.commentId + "/like")
        .then(({ data }) => {
          if (data.status) {
            // 判断是否点赞
    ...
    reloadReply(index) {
      this.axios
        .get("/api/comments/" + this.commentList[index].commentId + "/replies", {
          params: {
