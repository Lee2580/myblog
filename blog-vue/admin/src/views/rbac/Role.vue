...
export default {
  ...
  methods: {
    changeDisable(role) {
      this.axios.put("/api/admin/roles/status", {
        id: role.roleId,
        status: role.status
      });
    },
    ...
    deleteRoles(id) {
      ...
      this.axios.delete("/api/admin/deleteRoles", param).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
    ...
    saveOrUpdateRoleResource() {
      ...
      this.axios.post("/api/admin/saveOrUpdateRole", this.roleForm).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
            message: data.message
