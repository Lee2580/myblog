      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="scope">
          ...
          <el-popconfirm
            ...
            @confirm="deleteResource(scope.row.resourceId)"
          >
...
export default {
  ...
  methods: {
    ...
    changeResource(resource) {
      this.axios.post("/api/admin/saveOrUpdateResource", resource).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
    ...
    openAddResourceModel(resource) {
      ...
      this.resourceForm = {};
      this.resourceForm.parentId = resource.resourceId;
      this.$refs.resourceTitle.innerHTML = "添加资源";
      this.addResource = true;
    },
    deleteResource(resourceId) {
      this.axios.delete("/api/admin/resources/" + resourceId).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
            message: data.message
    ...
    addOrEditResource() {
      ...
      this.axios
        .post("/api/admin/saveOrUpdateResource", this.resourceForm)
        .then(({ data }) => {
          if (data.status) {
            this.$notify.success({
              title: "成功",
